...
Azure includes many services that can be used in a big data architecture. In practice, the technology choice is often between two broad sets of technology:
...
- **Complexity**. Big data solutions can be extremely complex, with numerous components to handle data ingestion from multiple data sources. It can be challenging to build, test, and troubleshoot big data processes. Morever, there may be a large number of configuration settings across multiple systems, which must be used in order to optimize performance.
